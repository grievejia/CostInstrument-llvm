/*
 *	ProgramOptions.hxx - a single-header program options parsing library for
 *C++11 Copyright (C) 2017  Josua Rieder (josua.rieder1996@gmail.com)
  ...
  do {                                                                         \
    if (!(Expression))                                                         \
      throw std::logic_error{                                                  \
          ("ProgramOptions.hxx:" + std::to_string(__LINE__) + ": ") +          \
          (Message)};                                                          \
  } while (0)
#endif // PROGRAMOPTIONS_NO_EXCEPTIONS
#endif // NDEBUG
...
class color_resetter {
  ...
  color_resetter(std::ostream& stream, color_t color) : m_stream{stream} {
#ifdef PROGRAMOPTIONS_WINDOWS
    m_stream << std::flush;
    ...
    if (color == maroon || color == brown || color == purple ||
        color == light_gray)
      attribute |= FOREGROUND_RED;
    if (color == green || color == brown || color == teal ||
        color == light_gray)
      attribute |= FOREGROUND_GREEN;
    if (color == navy || color == purple || color == teal ||
        color == light_gray)
...
inline std::ostream& operator<<(std::ostream& stream,
                                color_t /* color */) { // -Wunused-parameter
  return stream;
}
#endif // !PROGRAMOPTIONS_NO_COLORS
...
struct integer_sequence_assembler<T, i, integer_sequence<T, j...>,
...
inline std::size_t damerau_levenshtein(
    char const* a, char const* b, std::size_t i, std::size_t j,
...
public:
  ...
  friend std::ostream& operator<<(std::ostream& stream, repeat const& object) {
    std::fill_n(std::ostreambuf_iterator<char>(stream), object.m_count,
                object.m_character);
    return stream;
  }
};
...
struct vt2type_impl {};
template <>
struct vt2type_impl<void_> {
  using type = void_t;
};
template <>
struct vt2type_impl<string> {
  using type = string_t;
};
template <>
struct vt2type_impl<i32> {
  using type = i32_t;
};
template <>
struct vt2type_impl<i64> {
  using type = i64_t;
};
template <>
struct vt2type_impl<u32> {
  using type = u32_t;
};
template <>
struct vt2type_impl<u64> {
  using type = u64_t;
};
template <>
struct vt2type_impl<f32> {
  using type = f32_t;
};
template <>
struct vt2type_impl<f64> {
  using type = f64_t;
};
} // namespace detail
template <value_type type>
...
struct log10
    : public std::integral_constant<unsigned, 1 + log10<i / 10>::value> {};
template <>
struct log10<0, true>;
template <unsigned i>
...
struct parsing_report {
  ...
  parsing_report(error_code error) : error{error} {}
  parsing_report(T const& value) : value{value} {}
  parsing_report(T&& value) : value{std::move(value)} {}

  bool good() const { return error == error_code::none; }
  bool operator!() const { return !good(); }

  operator T const&() const {
    PROGRAMOPTIONS_ASSERT(
        good(), "parsing_report: cannot access data of an erroneous report");
    return value;
  }
};
...
bool expect_impl(forward_iterator_t /* first */,
                 forward_iterator_t /* last */) { // -Wunused-parameter
  return true;
}
template <typename forward_iterator_t, typename... tail_t>
bool expect_impl(forward_iterator_t& first, forward_iterator_t last, char head,
                 tail_t const&... tail) {
...
bool expect(forward_iterator_t& first, forward_iterator_t last,
            args_t&&... args) {
  forward_iterator_t first_copy{first};
  const bool result =
      detail::expect_impl(first_copy, last, std::forward<args_t>(args)...);
  if (result)
    first = first_copy;
  return result;
}
...
bool expect_unpacker(forward_iterator_t& first, forward_iterator_t last,
                     const char (&args)[N],
...
bool expect(forward_iterator_t& first, forward_iterator_t last,
            const char (&args)[N]) {
  return detail::expect_unpacker(first, last, args,
...
parsing_report<T> str2uint(forward_iterator_t first, forward_iterator_t last) {
  ...
  if (expect(first, last, 'x')) {
    ...
    for (int d{}; first != last && digits <= max_digits &&
                  (d = get_hex_digit(*first)) >= 0;
         ++first, ++digits)
      (result <<= 4) |= d;
    ...
    for (int d{}; first != last && decimals <= max_decimals &&
                  (d = get_digit(*first)) >= 0;
         ++first, ++decimals)
      result = 10 * result + static_cast<T>(d);
    if (decimals >= max_decimals)
      if (decimals > max_decimals ||
          (result < pow10<T, max_decimals - 1>::value))
        return error_code::out_of_range;
...
parsing_report<T> str2int(forward_iterator_t first, forward_iterator_t last) {
  ...
  if (!report)
    return report.error;
  unsigned_t max = neg ? static_cast<unsigned_t>(-std::numeric_limits<T>::min())
...
parsing_report<T> str2flt(forward_iterator_t first, forward_iterator_t last) {
  ...
  static_assert(std::numeric_limits<T>::is_iec559,
                "platform doesn't meet requirements");
...
struct value {
  ...
  value() = default;
  explicit value(string_t const& object) : string{object} {}
  explicit value(string_t&& object) : string{std::move(object)} {}
};
...
struct value_vector_base {
  ...
  value* begin() {
    return const_cast<value*>(
        static_cast<value_vector_base const&>(*this).begin());
  }

  value const* end() const { return begin() + size(); }
  value* end() {
    return const_cast<value*>(
        static_cast<value_vector_base const&>(*this).end());
  }
  ...
  value& get() {
    return const_cast<value&>(
        static_cast<value_vector_base const&>(*this).get());
  }
...
public:
  ...
  virtual void
  push_back(value const& /* object */) override { // -Wunused-parameter
    ++m_counter;
  }
};
template <>
...
class is_invocable_sfinae {
  ...
  template <typename _fun_t, typename... _args_t>
  static true_t&
  test(decltype(std::declval<_fun_t>()(std::declval<_args_t>()...)) *);

public:
  static constexpr bool value =
      sizeof(test<fun_t, args_t...>(0)) == sizeof(true_t);
};
} // namespace detail
template <typename fun_t, typename... args_t>
struct is_invocable
    : public std::integral_constant<
          bool, detail::is_invocable_sfinae<fun_t, args_t...>::value> {};
...
protected:
  typename std::remove_const<
      typename std::remove_reference<invocable_t>::type>::type m_invocable;
...
template <value_type type, typename invocable_t,
...
struct callback : public callback_storage<invocable_t> {
  ...
  virtual void
  invoke(value const& /* object */) override { // -Wunused-parameter
...
struct callback<string, invocable_t, true, with_string, false>
    : public callback_storage<invocable_t> {
  using callback_storage<invocable_t>::callback_storage;
  virtual void
  invoke(value const& /* object */) override { // -Wunused-parameter
...
struct callback<i32, invocable_t, with_void, true, false>
    : public callback_storage<invocable_t> {
  using callback_storage<invocable_t>::callback_storage;
  virtual void invoke(value const& object) override {
    this->m_invocable(object.string);
...
template <typename invocable_t>
struct callback<i32, invocable_t, true, false, false>
    : public callback_storage<invocable_t> {
  using callback_storage<invocable_t>::callback_storage;
  virtual void
  invoke(value const& /* object */) override { // -Wunused-parameter
...
template <typename invocable_t, bool with_void>
struct callback<i64, invocable_t, with_void, true, false>
    : public callback_storage<invocable_t> {
  using callback_storage<invocable_t>::callback_storage;
  virtual void invoke(value const& object) override {
    this->m_invocable(object.string);
...
template <typename invocable_t>
struct callback<i64, invocable_t, true, false, false>
    : public callback_storage<invocable_t> {
  using callback_storage<invocable_t>::callback_storage;
  virtual void
  invoke(value const& /* object */) override { // -Wunused-parameter
...
template <typename invocable_t, bool with_void>
struct callback<u32, invocable_t, with_void, true, false>
    : public callback_storage<invocable_t> {
  using callback_storage<invocable_t>::callback_storage;
  virtual void invoke(value const& object) override {
    this->m_invocable(object.string);
...
template <typename invocable_t>
struct callback<u32, invocable_t, true, false, false>
    : public callback_storage<invocable_t> {
  using callback_storage<invocable_t>::callback_storage;
  virtual void
  invoke(value const& /* object */) override { // -Wunused-parameter
...
template <typename invocable_t, bool with_void>
struct callback<u64, invocable_t, with_void, true, false>
    : public callback_storage<invocable_t> {
  using callback_storage<invocable_t>::callback_storage;
  virtual void invoke(value const& object) override {
    this->m_invocable(object.string);
...
template <typename invocable_t>
struct callback<u64, invocable_t, true, false, false>
    : public callback_storage<invocable_t> {
  using callback_storage<invocable_t>::callback_storage;
  virtual void
  invoke(value const& /* object */) override { // -Wunused-parameter
...
template <typename invocable_t, bool with_void>
struct callback<f32, invocable_t, with_void, true, false>
    : public callback_storage<invocable_t> {
  using callback_storage<invocable_t>::callback_storage;
  virtual void invoke(value const& object) override {
    this->m_invocable(object.string);
...
template <typename invocable_t>
struct callback<f32, invocable_t, true, false, false>
    : public callback_storage<invocable_t> {
  using callback_storage<invocable_t>::callback_storage;
  virtual void
  invoke(value const& /* object */) override { // -Wunused-parameter
...
template <typename invocable_t, bool with_void>
struct callback<f64, invocable_t, with_void, true, false>
    : public callback_storage<invocable_t> {
  using callback_storage<invocable_t>::callback_storage;
  virtual void invoke(value const& object) override {
    this->m_invocable(object.string);
...
template <typename invocable_t>
struct callback<f64, invocable_t, true, false, false>
    : public callback_storage<invocable_t> {
  using callback_storage<invocable_t>::callback_storage;
  virtual void
  invoke(value const& /* object */) override { // -Wunused-parameter
...
class value_iterator {
  ...
  static_assert(
      std::is_same<
          typename std::iterator_traits<underlying_t>::iterator_category,
...
public:
  using difference_type =
      typename std::iterator_traits<underlying_t>::difference_type;
...
template <value_type type, typename underlying_t>
value_iterator<type, underlying_t>
operator+(typename value_iterator<type, underlying_t>::difference_type n,
...
private:
  ...
  value_vector_base& get_vector() const {
    PROGRAMOPTIONS_ASSERT(
        m_data != nullptr || m_fallback != nullptr,
...
public:
  // using value_type = value; // there's another crucial type called
  // value_type...
  ...
  option& abbreviation(char value) {
    PROGRAMOPTIONS_ASSERT(
        value == '\0' || (std::isgraph(value) && value != '-'),
...
private:
  static bool valid_type(value_type type) {
    switch (type) { // switch( enum ) ensures that a warning is generated
                    // whenever an additional enum value is added
      case void_:
      case string:
      case i32:
      case i64:
...
public:
  option& type(value_type type) {
    PROGRAMOPTIONS_ASSERT(
        m_fallback == nullptr && m_data == nullptr && m_callbacks.empty(),
...
private:
  ...
  parsing_report<value> fallback_parse(T&& arg) {
    ...
    PROGRAMOPTIONS_ASSERT(result.good(),
                          "fallback: cannot convert argument to option's type");
    return result;
  }

  template <typename head_t, typename... tail_t>
  void fallback_single(head_t&& head,
                       tail_t&&... /* tail */) { // -Wunused-parameter
    PROGRAMOPTIONS_ASSERT(sizeof...(tail_t) == 0,
                          "fallback: too many arguments for single() option");
    decltype(m_fallback) new_fallback =
        make_unique<value_vector<false, true>>();
    new_fallback->push_back(fallback_parse(std::forward<head_t>(head)));
    m_fallback.swap(new_fallback);
  }

  template <typename head_t, typename... tail_t>
  void fallback_helper(value_vector_base& vector, head_t&& head,
                       tail_t&&... tail) {
  ...
  void fallback_multi(args_t&&... args) {
    decltype(m_fallback) new_fallback =
        make_unique<value_vector<false, false>>();
...
public:
  ...
  option& fallback(args_t&&... args) {
    ...
    PROGRAMOPTIONS_ASSERT(get_type() != void_,
                          "fallback: not allowed for options of type void_");
    mutable_operation();
    if (is_single())
  ...
  option& callback(invocable_t&& invocable) {
    ...
    switch (get_type()) {
      case void_:
        new_callback = make_unique<::po::callback<
            void_, typename std::remove_reference<invocable_t>::type>>(
            std::forward<invocable_t>(invocable));
        break;
      case string:
        new_callback = make_unique<::po::callback<
            string, typename std::remove_reference<invocable_t>::type>>(
            std::forward<invocable_t>(invocable));
        break;
      case i32:
        new_callback = make_unique<::po::callback<
            i32, typename std::remove_reference<invocable_t>::type>>(
            std::forward<invocable_t>(invocable));
        break;
      case i64:
        new_callback = make_unique<::po::callback<
            i64, typename std::remove_reference<invocable_t>::type>>(
            std::forward<invocable_t>(invocable));
        break;
      case u32:
        new_callback = make_unique<::po::callback<
            u32, typename std::remove_reference<invocable_t>::type>>(
            std::forward<invocable_t>(invocable));
        break;
      case u64:
        new_callback = make_unique<::po::callback<
            u64, typename std::remove_reference<invocable_t>::type>>(
            std::forward<invocable_t>(invocable));
        break;
      case f32:
        new_callback = make_unique<::po::callback<
            f32, typename std::remove_reference<invocable_t>::type>>(
            std::forward<invocable_t>(invocable));
        break;
      case f64:
        new_callback = make_unique<::po::callback<
            f64, typename std::remove_reference<invocable_t>::type>>(
            std::forward<invocable_t>(invocable));
        // 	break;
        // default:
        // 	assert( false );
...
#ifdef PROGRAMOPTIONS_DEBUG
    PROGRAMOPTIONS_ASSERT(new_callback->good(),
                          "callback: incompatible parameter type");
#endif // PROGRAMOPTIONS_DEBUG
...
private:
  ...
  void error_nonoption_arguments(arg_t const& arg) {
    assert(is_verbose());
    *m_output_destination << error() << "non-option arguments not allowed"
                          << ignoring(arg);
  }
  template <typename arg_t>
  ...
  void check_spelling(char const* long_option) {
    assert(is_verbose());
    assert(long_option[0] == '-' && long_option[1] == '-' &&
           long_option[2] != '\0');
    ...
    for (auto iter = m_options.begin(); iter != m_options.end(); ++iter) {
      ...
      const std::size_t distance =
          damerau_levenshtein(name_begin, iter->first.c_str(), length,
                              iter->first.length(), distance_cutoff);
  ...
  bool parse_argument(options_t::iterator option,
                      expression_t const& expression, args_t&&... args) const {
      ...
      *m_output_destination << "\' ";
      switch (code) {
        ...
        case error_code::out_of_range:
          *m_output_destination
              << "has an argument that caused an out of range error";
        default: // -Wswitch
                 ;
      }
  ...
  bool extract_argument(options_t::iterator option, std::size_t argc,
                        char** argv, std::size_t& i, std::size_t j) {
    ...
    if (argv[i][j] == '\0') {
      ...
      if (is_verbose())
        *m_output_destination << error() << "unexpected character \'"
                              << argv[i][j] << "\'" << ignoring(argv[i])
                              << suggest(std::string{&argv[i][0], &argv[i][j]} +
                                         "=" + std::string{&argv[i][j]})
                              << '\n';
      return false;
    }
  ...
  bool good_unnamed_argument(options_t::iterator option, bool& good,
                             char const* arg) {
...
public:
  bool wellformed() const {
    ...
    if (std::adjacent_find(abbreviations.begin(), abbreviations.end()) !=
        abbreviations.end())
      return false;
    return true;
  }
  ...
  bool operator()(int int_argc, char** argv) {
    PROGRAMOPTIONS_ASSERT(wellformed(),
                          "cannot parse with an ill-formed parser");
    PROGRAMOPTIONS_ASSERT(std::none_of(m_options.begin(), m_options.end(),
...
#ifdef PROGRAMOPTIONS_WINDOWS
    m_program_name =
        std::max(std::strrchr(argv[0], '/'), std::strrchr(argv[0], '\\'));
    ...
    for (std::size_t i = 1; i < argc; ++i) {
      if (argv[i][0] == '\0')
        continue;
      if (argv[i][0] != '-' ||
          (unnamed != m_options.end() && dashed_non_option(argv[i]))) {
          ...
          if (head == m_options.end()) {
            ...
            for (std::size_t j = 1; (c = argv[i][j]) != '\0'; ++j) {
              if (!std::isprint(c) || c == '-') {
                good = false;
                if (is_verbose())
                  *m_output_destination << error() << "invalid character \'"
                                        << c << "\'" << ignoring(&argv[i][j])
                                        << '\n';
                break;
              }
              ...
              if (opt->second.get_type() != void_) {
                ...
                if (is_verbose())
                  *m_output_destination
                      << error()
                      << "non-void options not allowed in option packs"
                      << ignoring(c) << '\n';
                continue;
              }
...
private:
  ...
  static bool valid_designator(std::string const& designator) {
    ...
    if (designator[0] == '-')
      return false;
    return std::find_if_not(designator.begin(), designator.end(),
                            &valid_designator_character) == designator.end();
  }
  option& operator_brackets_helper(std::string&& designator) {
    PROGRAMOPTIONS_ASSERT(valid_designator(designator),
                          "operator[]: designator may only consist of letters, "
                          "hyphens and underscores and mustn't start with a "
                          "hyphen");
...
public:
  ...
  friend std::ostream& operator<<(std::ostream& stream, parser const& object) {
    enum : std::size_t {
      console_width =
          80 - 1, // -1 because writing until the real end returns the carriage
      left_padding = 2,
    ...
    PROGRAMOPTIONS_ASSERT(object.wellformed(),
                          "cannot print an ill-formed parser");
    ...
    if (any_verbose)
      max_verbose += 2; // --
    const std::size_t abbreviation_width =
        any_abbreviations * max_abbreviation_width;
    const std::size_t separator_width =
        (any_abbreviations && any_verbose) * max_separator_width;
    const std::size_t verbose_start =
        left_padding + abbreviation_width + separator_width;
    const std::size_t verbose_width =
        std::min(any_verbose * max_verbose_width, max_verbose);
    const std::size_t description_start =
        verbose_start + verbose_width + mid_padding;
    for (auto&& i : object.m_options) {
      if (i.first.empty())
        continue;
      ...
      for (std::size_t i = 0; i < descr.size(); ++i) {
        const bool last =
            (i + 1 < descr.size()) && (carriage + 1 >= console_width);
        if (last) {
